...
public class SearchTabsController extends BaseFormController {
    private static TWTNavSection searchNavSection;
    public static TWTNavSection getSearchNavSection() {
        if (searchNavSection == null) {
            searchNavSection = new TWTNavSection(action(icon(FontImage.MATERIAL_SEARCH))) {
                @Override
                public FormController createFormController(Controller parent) {
                    return new SearchTabsController(parent);
                }
            };
        }

        return searchNavSection;
    }
    ...
    public SearchTabsController(Controller parent) {
        ...
        CollapsibleHeaderContainer wrapper = new CollapsibleHeaderContainer(
                new TWTTitleComponent((Entity)lookup(IApplicationModel.class), getViewNode(), new TWTSearchButton((Entity)lookup(IApplicationModel.class), getViewNode())),
                view, 
                view
        );
        setView(wrapper);
    ...
    public void initController() {
        ...
        lookupEntity(IApplicationModel.class).set(IApplicationModel.currentSection, getSearchNavSection());
    }
    ...
    private TWTNewsItem createItem(Entity creator, Date date, String headline, String thumbnailUrl) {
        TWTNewsItem item = new TWTNewsItem();
        item.setEntity(INewsItem.creator, creator);
        item.setDate(INewsItem.date, date);
        item.setText(INewsItem.headline, headline);
        item.setText(INewsItem.thumbnailUrl, thumbnailUrl);
        item.setText(INewsItem.image, thumbnailUrl);
        return item;
    }
